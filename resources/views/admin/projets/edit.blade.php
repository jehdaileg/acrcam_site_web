...
@section('main_content_admin_part')
    ...
    <h5 class="text-gray-800 text-xl font-bold mt-3">Modification du projet</h5>
    ...
    <form method="POST" action="{{ route('projets.update', $projet) }}" enctype="multipart/form-data">
        ...
        <div class="mb-6">
          ...
          <input type="text" value="{{ $projet->name }}" name="name" placeholder="Saisissez le nom du projet"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
        </div>
        ...
        <div class="mb-6">
          ...
          <div class="mb-8">
            ...
            <select name="type_projet_id" id="underline_select" class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                ...
                <option value="{{ $type_projet->id }}" {{ $type_projet->id == $projet->type_projet_id ? 'selected': '' }}>{{ $type_projet->nameTypeProjet }}</option>
                @endforeach
          ...
          <div class="mb-6">
            ...
            <input type="date" value="{{ $projet->date_realisation }}" name="date_realisation" placeholder="Saisissez la date de réalisation du projet"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
          </div>


          <div class="mb-6">
            <label for="" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nom du partenaire:</label>
            <input type="text" value="{{ $projet->partenaire_concerne }}" name="partenaire_concerne" placeholder="Saisissez le nom du partenaire dans le projet"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
          </div>

          <div class="mb-6">
            <label for="" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Estimer un revenu du projet:</label>
            <input type="number" value="{{ $projet->revenu_benefice_projet }}" name="revenu_benefice_projet" placeholder="Saisissez une marge de revenu du projet"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
          </div>
          ...
          <select name="statut_projet" id="underline_select" class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                ...
                @foreach($projet->getStatusProjet() as $key => $value)
                <option value="{{ $key }}" {{ $projet->statut_projet == $value ? 'selected': '' }}>{{$value }}</option>
                @endforeach
